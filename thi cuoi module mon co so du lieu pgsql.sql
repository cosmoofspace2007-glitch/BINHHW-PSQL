...
--3. Cập nhật dữ liệu (6 điểm) Viết câu lệnh UPDATE để cập nhật lại total_amount trong bảng Booking theo công thức: total_amount = total_amount * 0.9
-- cho những bản ghi có ngày check_in trước ngày 3/3/2025.
update booking
set total_amount = total_amount * 0.9
where date(booking.check_in_date) < '2025-03-03';
...
--(3 điểm) Lấy thông tin khách hàng gồm: mã khách hàng, họ tên, email, số điện thoại được sắp xếp theo họ tên khách hàng giảm dần.
select customer_id,customer_full_name,customer_email,customer_phone from customer order by customer_full_name desc ;
--(3 điểm) Lấy thông tin các phòng khách sạn gồm: mã phòng, loại phòng, giá phòng và diện tích phòng, sắp xếp theo diện tích phòng tăng dần
select room_id,room_type,room_price,room_area from room order by room_area asc;
--(3 điểm) Lấy thông tin khách hàng và phòng khách sạn đã đặt gồm: họ tên khách hàng, mã phòng, ngày nhận phòng và ngày trả phòng.
...
--(3 điểm) Lấy danh sách khách hàng và tổng tiền đã thanh toán khi đặt phòng,
-- gồm mã khách hàng, họ tên khách hàng, phương thức thanh toán và số tiền thanh toán, sắp xếp theo số tiền thanh toán tăng dần.
select c.customer_id,customer_full_name,payment_method,payment_amount
from  customer c join booking b on c.customer_id = b.customer_id join payment p on b.booking_id = p.booking_id
order by payment_amount asc;
--(3 điểm) Lấy tất cả thông tin khách hàng từ vị trí thứ 2 đến thứ 4 trong bảng Customer được sắp xếp theo tên khách hàng (Z-A).
select  * from customer order by customer_full_name desc offset 1 limit 4;
--(5 điểm) Lấy danh sách khách hàng đã đặt ít nhất 2 phòng gồm : mã khách hàng, họ tên khách hàng và số lượng phòng đã đặt.
select b.customer_id,customer_full_name, count(booking_id)
from customer c join booking b on c.customer_id = b.customer_id
group by b.customer_id,customer_full_name
having count(booking_id) >= 2;
--(5 điểm) Lấy danh sách các phòng từng có ít nhất 3 khách hàng đặt, gồm mã phòng, loại phòng, giá phòng và số lần đã đặt.
select b.room_id,room_type,count(booking_id)
from booking b join room r on b.room_id = r.room_id
group by b.room_id,room_type
having count(booking_id) <= 3;
--(5 điểm) Lấy danh sách các khách hàng có tổng số tiền đã thanh toán lớn hơn 1000, gồm mã khách hàng, họ tên khách hàng, mã phòng, tổng số tiền đã thanh toán.
select b.customer_id,customer_full_name,b.room_id,sum(payment_amount)
from customer c join booking b on c.customer_id = b.customer_id
    join payment p on b.booking_id = p.booking_id
    join room r on r.room_id = b.room_id
group by customer_full_name, b.customer_id, b.room_id
having sum(payment_amount) > 1000;
--(6 điểm) Lấy danh sách các khách hàng gồm : mã KH, Họ tên, email, sđt có họ tên chứa chữ "Minh" hoặc địa chỉ ở "Hanoi". Sắp xếp kết quả theo họ tên tăng dần.
select * from customer
where customer_full_name ilike '%Minh%' or customer_address = 'Hanoi, Vietnam';
--(4 điểm)  Lấy danh sách thông tin các phòng gồm : mã phòng, loại phòng, giá , sắp xếp theo giá phòng giảm dần.
-- Chỉ lấy 5 phòng và bỏ qua 5 phòng đầu tiên (tức là lấy kết quả của trang thứ 2, biết mỗi trang có 5 phòng).
select room_id,room_type,room_price from room order by room_price desc
offset 5;


--PHẦN 3: Tạo View
--(5 điểm) Hãy tạo một view để lấy thông tin các phòng và khách hàng đã đặt, với điều kiện ngày nhận phòng nhỏ hơn ngày 2025-03-04.
-- Cần hiển thị các thông tin sau: Mã phòng, Loại phòng, Mã khách hàng, họ tên khách hàng
create or replace view info_cus as
select b.room_id,room_type,b.customer_id,customer_full_name
from customer c join booking b on c.customer_id = b.customer_id join room r on r.room_id = b.room_id
where check_in_date < '2025-03-04';
--(5 điểm) Hãy tạo một view để lấy thông tin khách hàng và phòng đã đặt, với điều kiện diện tích phòng lớn hơn 30 m².
-- Cần hiển thị các thông tin sau: Mã khách hàng, Họ tên khách hàng, Mã phòng, Diện tích phòng, Ngày nhận phòng
create or replace view info_cus_2 as
...
as $$
begin
    if new.check_in_day > new.check_out_day then
...
-- tự động cập nhật trạng thái phòng thành "Booked" khi một phòng được đặt (khi có bản ghi được INSERT vào bảng Booking).
...
create or replace procedure add_customer(IN in_customer_id INT, IN in_customer_full_name VARCHAR(100),IN in_customer_email VARCHAR(100)
,IN in_customer_phone VARCHAR(15), IN in_customer_address VARCHAR(255))
language plpgsql
as $$
begin
    insert into customer(customer_id, customer_full_name, customer_email, customer_phone, customer_address) values
                        (in_customer_id,in_customer_full_name,in_customer_email,in_customer_phone,in_customer_address);
end;
$$;
...
create or replace procedure add_payment(in p_booking_id int,in p_payment_method varchar(50),in p_payment_amount decimal(10,2),
